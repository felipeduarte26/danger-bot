...
interface InitializerAssignment {
  field: string;
  param: string;
  line: number;
}
...
interface ConstructorViolation {
  className: string;
  assignments: InitializerAssignment[];
  constructorLine: number;
}

function isGeneratedOrTestFile(file: string): boolean {
  return (
 ...
 */
function extractConstructorBlock(lines: string[], startIdx: number): string {
  let block = "";
  let parenDepth = 0;
  let braceDepth = 0;
 ...
 */
function findSimpleAssignments(
  initializerBlock: string,
  paramNames: Set<string>
): InitializerAssignment[] {
  const assignments: InitializerAssignment[] = [];
 ...
 */
function isFieldMatchingParam(field: string, param: string): boolean {
 ...
 */
function extractNamedParams(constructorBlock: string): Set<string> {
 ...
 */
function extractParamNameFromChunk(chunk: string): string | null {
  let text = chunk.trim();
 ...
 */
function hasThisPrivateParam(constructorBlock: string, fieldName: string): boolean {
 ...
 */
function extractInitializerList(constructorBlock: string): string {
  const parenStart = constructorBlock.indexOf("(");
  const parenEnd = findMatchingParen(constructorBlock, parenStart);
...
function findViolationsInFile(content: string): ConstructorViolation[] {
  const violations: ConstructorViolation[] = [];
  const lines = content.split("\n");

  const classPattern = /^\s*(?:final\s+|abstract\s+|base\s+|sealed\s+|mixin\s+)*class\s+(\w+)/;
  const constructorPattern = (className: string) =>
    new RegExp(`^\\s*(?:const\\s+)?${className}\\s*\\(`);
  const namedConstructorPattern = (className: string) =>
...
export default createPlugin(
  ...
  async () => {
    ...
    const files = [...git.created_files, ...git.modified_files].filter(
      (f: string) => f.endsWith(".dart") && !isGeneratedOrTestFile(f) && fs.existsSync(f)
    );
