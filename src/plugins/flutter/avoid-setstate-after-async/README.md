...
> Detecta chamadas de setState apos await sem verificar mounted
...
```typescript
import { avoidSetstateAfterAsyncPlugin } from "@felipeduarte26/danger-bot";
```
...
```typescript
// dangerfile.ts
import { avoidSetstateAfterAsyncPlugin, executeDangerBot } from "@felipeduarte26/danger-bot";

executeDangerBot([
  avoidSetstateAfterAsyncPlugin, // Habilitado por padrão
]);
```
...
avoidSetstateAfterAsyncPlugin.config.enabled = false;

// Modificar configuração
avoidSetstateAfterAsyncPlugin.config.description = "Minha descrição customizada";
```
...
```
✅ Avoid Setstate After Async: Todas as verificações passaram!
```

### ⚠️ Quando problemas são encontrados

```
⚠️ Avoid Setstate After Async: Verificação detectou problemas
...
```
❌ Avoid Setstate After Async: Erro crítico detectado
...
if (isMainBranch) {
  avoidSetstateAfterAsyncPlugin.config.enabled = true;
}
```
...
- Use o CLI para validar: `danger-bot validate src/plugins/avoid-setstate-after-async/avoid-setstate-after-async.ts`
