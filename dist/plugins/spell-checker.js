...
exports.default = (0, types_1.createPlugin)({
    ...
    try {
        ...
        const dangerBotPath = require.resolve("@genial-solutions/danger-bot").replace(/dist.*$/, "");
        ...
        if (!fs.existsSync("temp_identifiers_for_spell_check.txt") ||
        ...
        let cspellOutput = "";
        try {
            (0, child_process_1.execSync)("./node_modules/.bin/cspell --config cspell.config.json --no-progress --no-summary temp_identifiers_for_spell_check.txt", { encoding: "utf-8", stdio: "pipe" });
        }
        catch (error) {
        ...
        if (errors.length > 0) {
            for (const err of errors) {
                ...
                fail(`**ERRO ORTOGRÁFICO**: \`${err.word}\` em ${typeMap[err.type] || err.type} \`${err.identifier}\`\n\n` +
                    `\`\`\`dart\n${err.context}\n\`\`\`\n\n` +
                    `**Ação**: Corrija ou adicione ao dicionário (.vscode/settings.json)`, err.file, err.line);
            }
            message(`⚠️ ${errors.length} erro(s) ortográfico(s) em ${new Set(errors.map((e) => e.file)).size} arquivo(s)`);
        }
        else {
            message("✅ **cspell**: Nenhum erro ortográfico!");
        }
        // Cleanup
        ["temp_identifiers_for_spell_check.txt", "temp_spell_check_metadata.json", "cspell.config.json"].forEach((f) => fs.existsSync(f) && fs.unlinkSync(f));
    }
    catch (error) {
...
function parseCspellOutput(output, metadata) {
    ...
    return errors.filter((e, i, self) => i ===
        self.findIndex((x) => x.originalFile === e.originalFile &&
            x.line === e.line &&
            x.word === e.word));
}
