...
function suggestName(name, firstWord) {
    const rest = name.startsWith("_")
        ? "_is" + name.slice(1).charAt(0).toUpperCase() + name.slice(2)
        : "is" + name.charAt(0).toUpperCase() + name.slice(1);
    const suggestions = {
        show: name.replace(new RegExp(`^_?${firstWord}`, "i"), (m) => m.startsWith("_") ? "_shouldShow" : "shouldShow"),
        hide: name.replace(new RegExp(`^_?${firstWord}`, "i"), (m) => m.startsWith("_") ? "_isHidden" : "isHidden"),
        delete: name.replace(new RegExp(`^_?${firstWord}`, "i"), (m) => m.startsWith("_") ? "_isDeleted" : "isDeleted"),
        enable: name.replace(new RegExp(`^_?${firstWord}`, "i"), (m) => m.startsWith("_") ? "_isEnabled" : "isEnabled"),
        disable: name.replace(new RegExp(`^_?${firstWord}`, "i"), (m) => m.startsWith("_") ? "_isDisabled" : "isDisabled"),
        load: name.replace(new RegExp(`^_?${firstWord}`, "i"), (m) => m.startsWith("_") ? "_isLoaded" : "isLoaded"),
        toggle: name.replace(new RegExp(`^_?${firstWord}`, "i"), (m) => m.startsWith("_") ? "_isToggled" : "isToggled"),
        connect: name.replace(new RegExp(`^_?${firstWord}`, "i"), (m) => m.startsWith("_") ? "_isConnected" : "isConnected"),
    };
...
exports.default = (0, _types_1.createPlugin)({
    ...
    for (const file of dartFiles) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (GOOD_PREFIXES.has(firstWord))
                continue;
            if (GOOD_ADJECTIVES.has(cleanName.toLowerCase()))
                continue;
            if (GOOD_ADJECTIVES.has(firstWord))
                continue;
