...
function findPublicMethodViolations(lines, classStartLine, classEndLine) {
    ...
    for (let i = classStartLine; i <= classEndLine; i++) {
        ...
        if (!classBodyStarted || depthBefore !== 1)
            continue;
        if (trimmed.length === 0 || isCommentOrAnnotation(trimmed))
            continue;
        if (trimmed.startsWith("static "))
            continue;
        if (/^\s*(get|set)\s+/.test(line))
            continue;
        if (isGetterDeclaration(line))
            continue;
        if (hasAssignmentBeforeParen(line))
            continue;
        ...
        if (SKIP_KEYWORDS.has(returnType))
            continue;
        if (SKIP_KEYWORDS.has(methodName))
            continue;
        if (methodName.startsWith("_"))
            continue;
        if (hasOverrideAnnotation(lines, i))
            continue;
...
exports.default = (0, _types_1.createPlugin)({
    ...
    const files = [...git.created_files, ...git.modified_files].filter((f) => (f.endsWith("_viewmodel.dart") || f.endsWith("_view_model.dart")) && fs.existsSync(f));
    for (const file of files) {
        const content = fs.readFileSync(file, "utf-8");
        if (!content.includes("extends ViewModelBase"))
            continue;
            ...
            if (insideClass && braceDepth <= 0 && line.includes("}")) {
                ...
                for (const v of violations) {
                    (0, _types_1.sendFormattedFail)({
                        ...
                        objective: "Métodos públicos de ViewModel devem apenas **disparar mudanças de estado**. Quem precisa do resultado acessa via `state`.",
                        reference: {
