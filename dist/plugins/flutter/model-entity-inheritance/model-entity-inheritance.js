...
function parseFields(lines, classStartLine) {
    const fields = [];
    let braceDepth = 0;
    let foundOpen = false;
...
function parseModelClass(content) {
                ...
                if (/\btoEntity\s*\(/.test(lines[k]) && !/\.toEntity\s*\(/.test(lines[k])) {
                    ...
                    if (returnMatch) {
                        ...
                        if (arrowMatch) {
                            ...
                            if (nextArrow)
                                toEntityReturnType = nextArrow[1];
                        }
                    }
                    break;
                }
...
function parseEntityClass(content) {
    const lines = content.split("\n");
    let inBlockComment = false;
    for (let i = 0; i < lines.length; i++) {
        const trimmed = lines[i].trimStart();
...
function findEntityFile(modelPath, snakeBaseName) {
...
function checkMissingInheritance(modelClass, entityClass, filePath) {
    ...
    if (!allFieldsMatch || matchedFields.length === 0)
        return;
    ...
    const fieldsSample = matchedFields
        ...
        .join("\n  ");
    const extraFields = matchedFields.length > 4 ? `\n  // ... e mais ${matchedFields.length - 4} campo(s)` : "";
...
function emitRedundantToEntity(modelClass, entityClass, filePath) {
    const entityName = entityClass.name;
    (0, _types_1.sendFormattedFail)({
...
exports.default = (0, _types_1.createPlugin)({
    ...
    const files = [...git.created_files, ...git.modified_files].filter((f) => f.includes("/data/") &&
        f.includes("/models/") &&
    ...
    for (const file of files) {
        ...
        if (!entityClass)
            continue;
        if (entityClass.fields.length === 0)
            continue;
