...
function isNonEnglishWord(word, eld) {
    if (NON_ENGLISH_FALLBACK.has(word.toLowerCase()))
        return true;
...
function extractIdentifiers(line, lineNumber) {
    ...
    if (!clean.trim())
        return results;
    const classMatch = clean.match(/(?:abstract\s+interface\s+class|abstract\s+class|final\s+class|sealed\s+class|class)\s+([A-Za-z_]\w*)/);
    if (classMatch) {
    ...
    const methodRe = /(?:Future<[^>]*>|void|String|int|double|bool|dynamic|List<[^>]*>|Map<[^,>]*,[^>]*>|Set<[^>]*>|[A-Z]\w*(?:<[^>]*>)?)\s+([a-z_]\w*)\s*\(/g;
    let m;
    while ((m = methodRe.exec(clean)) !== null) {
        const name = m[1];
        if (/^(get|set|build|createState|initState|dispose|toString|hashCode|main|runApp|of|from|parse|tryParse)$/.test(name))
            continue;
    ...
    const varRe = /(?:final|const|var|late\s+final|late)\s+(?:[A-Za-z_]\w*(?:<[^>]*>)?\s+)?([a-z_]\w*)\s*[=;]/g;
    while ((m = varRe.exec(clean)) !== null) {
        const name = m[1];
        if (/^(i|j|k|e|x|y|_)$/.test(name))
            continue;
        for (const word of breakCamelCase(name)) {
            results.push({
                word,
...
exports.default = (0, _types_1.createPlugin)({
    ...
    try {
            ...
            for (const chunk of diff.chunks) {
                for (const change of chunk.changes) {
                    ...
                    if (lineContent.trim().startsWith("export "))
                        continue;
                    if (lineContent.trim().startsWith("part "))
                        continue;
        ...
        let cspellOutput = "";
        try {
            (0, child_process_1.execSync)(`./node_modules/.bin/cspell --config ${configFile} --no-progress --no-summary ${wordsFile}`, { encoding: "utf-8", stdio: "pipe", timeout: 30000 });
        }
        catch (error) {
        ...
        for (const id of allIdentifiers) {
            if (!misspelled.has(id.word.toLowerCase()))
                continue;
        ...
        for (const [file, errors] of errorsByFile) {
            ...
            if (nonEnErrors.length > 0) {
                ...
                const items = nonEnErrors
                    .map((e) => `\`${e.word}\` em ${typeMap[e.type] || e.type} \`${e.identifier}\` (linha ${e.line})`)
                    .join("\n");
        ...
        if (totalErrors > 0) {
            (0, _types_1.sendMessage)(`**Spell Check**: ${totalErrors} problema(s) em ${errorsByFile.size} arquivo(s)`);
        }
    }
    finally {
        try {
