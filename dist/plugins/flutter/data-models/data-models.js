...
exports.default = (0, _types_1.createPlugin)({
    ...
    const files = [...git.created_files, ...git.modified_files].filter((f) => f.includes("/models/") &&
        f.endsWith(".dart") &&
    ...
    for (const file of files) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (!trimmed.match(/^\s*(?:abstract\s+|final\s+|sealed\s+|base\s+|interface\s+|mixin\s+)*class\s/)) {
                ...
                const classMatch = trimmed.match(/^(?:abstract\s+|final\s+|sealed\s+|base\s+|interface\s+|mixin\s+)*class\s+([A-Za-z_]\w*)/);
            ...
            if (classes.length > 0 && !hasNonFinalField) {
                const fieldMatch = line.match(/^\s+(?!final\s|static\s|const\s|late\s|@override)((?:String|int|double|bool|num|List|Map|Set|DateTime|[A-Z]\w*)[?<\s][\w<>,?\s]*)\s+(\w+)\s*;/);
                if (fieldMatch) {
