...
function checkBarrelCompleteness(barrelPath, dir) {
    ...
    const exportedLocalFiles = new Set();
    for (const exp of exportedPaths) {
        if (exp.startsWith(".") || exp.startsWith("/") || exp.startsWith("package:"))
            continue;
    ...
    const dartFiles = dirEntries.filter((e) => e.isFile() && e.name.endsWith(".dart") && e.name !== barrelFileName && !isIgnoredFile(e.name));
    const missingFiles = [];
    for (const entry of dartFiles) {
        if (exportedLocalFiles.has(entry.name))
            continue;
        const fullPath = path.join(dir, entry.name);
        if (isPartFile(fullPath))
            continue;
    ...
    (0, _types_1.sendFormattedFail)({
        ...
        objective: "Barrel file deve exportar **todos** os arquivos da pasta — evita imports diretos inconsistentes.",
        reference: {
...
exports.default = (0, _types_1.createPlugin)({
    ...
    for (const file of dartFiles) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (DART_CORE_RE.test(line))
                continue;
            if (RELATIVE_RE.test(line))
                continue;
            const match = line.match(IMPORT_RE);
            if (match) {
                if (!match[1].startsWith(`${projectPackage}/`))
                    continue;
                packageImports.push({ path: match[1], line: i + 1 });
            }
        }
        if (packageImports.length < 2)
            continue;
        const groups = groupByFolder(packageImports);
        for (const group of groups) {
            if (group.imports.length < 2)
                continue;
            const folderName = group.folder.split("/").pop() || group.folder;
            const barrelName = `${folderName}.dart`;
            const alreadyUsesBarrel = group.imports.some((imp) => imp.endsWith(`/${barrelName}`) || imp.endsWith(`/${folderName}/${folderName}.dart`));
            if (alreadyUsesBarrel)
                continue;
    ...
    for (const file of dartFiles) {
        ...
        for (const imp of directImports) {
            ...
            if (!fs.existsSync(barrelDiskPath))
                continue;
            ...
            const isExportedDirectly = Array.from(barrelExports).some((exp) => exp === importedFileName || exp.endsWith(`/${importedFileName}`));
            if (!isExportedDirectly)
                continue;
            const barrelImportPath = `${projectPackage}/${barrelRelPath}`;
            const alreadyImportsBarrel = lines.some((l) => l.trim().match(IMPORT_RE)?.[1] === barrelImportPath);
            if (alreadyImportsBarrel)
                continue;
            (0, _types_1.sendFormattedFail)({
                ...
                objective: "Barrel files **centralizam exports** — imports diretos quebram essa organização.",
                reference: {
                    text: "Guia completo sobre Barrel Files",
                    url: "https://medium.com/@ugamakelechi501/barrel-files-in-dart-and-flutter-a-guide-to-simplifying-imports-9b245dbe516a",
                },
