...
function analyzeBlock(block) {
    ...
    for (let i = 0; i < sizedBoxIndices.length - 1; i++) {
        if (sizedBoxIndices[i + 1] - sizedBoxIndices[i] < 2)
            return null;
    }
    ...
    if (sizedBoxValues.length !== expectedSizedBoxCount)
        return null;
    for (let i = 0; i < children.length; i++) {
        const isSizedBox = sizedBoxIndices.includes(i);
        const shouldBeSizedBox = i % 2 === 1;
        if (isSizedBox !== shouldBeSizedBox)
            return null;
    }
...
exports.default = (0, _types_1.createPlugin)({
    ...
    description: "Sugere uso de spacing em Column/Row ao invés de SizedBox intercalados (Flutter 3.27+)",
    enabled: true,
}, async () => {
    ...
    for (const file of dartFiles) {
        ...
        if (!content.includes("SizedBox"))
            continue;
        if (!content.includes("Column") && !content.includes("Row"))
            continue;
        ...
        for (const block of blocks) {
            ...
            (0, _types_1.sendFormattedFail)({
                ...
                objective: "Usar a propriedade **spacing** do Flutter 3.27+ para código mais limpo e menos widgets na árvore.",
                reference: {
