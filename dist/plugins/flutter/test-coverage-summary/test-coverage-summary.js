...
function reportCoverage(git) {
    ...
    if (!fs.existsSync(lcovPath)) {
        ...
        if (!generated) {
            (0, _types_1.sendMarkdown)("📊 **Cobertura de Testes** — indisponível\n\n" +
    ...
    const prFiles = new Set([...git.created_files, ...git.modified_files]
    ...
    if (prCoverage.length === 0) {
        ...
        (0, _types_1.sendMarkdown)(`📊 **Cobertura de Testes** — ${coverageEmoji(totalPercent)} ${totalPercent}% geral ` +
    ...
    const totalPercent = totalLines > 0 ? Math.round((totalHit / totalLines) * 100) : 0;
    let md = `${coverageEmoji(totalPercent)} **Cobertura de Testes** — ` +
...
function sendChecklist(git) {
    const hasDart = [...git.created_files, ...git.modified_files].some((f) => f.endsWith(".dart") && !f.endsWith("_test.dart"));
    if (!hasDart)
        return;
    let md = "**Checklist**\n\n";
    md += "| | Item |\n";
