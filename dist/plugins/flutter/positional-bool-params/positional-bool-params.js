...
exports.default = (0, _types_1.createPlugin)({
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (!inSignature) {
                if (trimmed.startsWith("//") ||
                    ...
                    trimmed.startsWith('r"'))
                    continue;
                const funcStart = trimmed.match(/(?:(?:static|abstract|external|factory)\s+)*(?:(?:Future|FutureOr|Stream|void|bool|int|double|String|num|dynamic|[\w<>,?]+)\s+)?(\w+)\s*\(/);
                if (!funcStart) {
                ...
                if (parenDepth <= 0) {
                    inSignature = false;
                    const funcMatch = signatureBuffer.match(/(?:(?:static|abstract|external|factory)\s+)*(?:(?:Future|FutureOr|Stream|void|bool|int|double|String|num|dynamic|[\w<>,?]+)\s+)?(\w+)\s*\(/);
    ...
    for (const v of violations) {
        ...
        (0, _types_1.sendFormattedFail)({
            ...
            objective: "Seguir **Effective Dart** — `Task(true)` é confuso, `Task(repeating: true)` é claro.",
            reference: {
...
function analyzeSignature(signature, file, startLine, functionName, violations) {
    ...
    while ((match = boolParamRegex.exec(paramsStr)) !== null) {
        ...
        if (isInsideNamedParams(paramsStr, paramIndex))
            continue;
