...
function getSingularForm(word) {
    if (!compromiseLib)
        return word;
    const lower = word.toLowerCase();
    return (compromiseLib("the " + lower)
        .nouns()
        .toSingular()
        .text()
        .replace("the ", "") || lower);
...
function isTargetFile(file) {
    ...
    if (file.endsWith("_viewmodel.dart") || file.endsWith("_view_model.dart"))
        return "ViewModel";
    return null;
}
    ...
    if (wp) {
        ...
        for (const file of targetFiles) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (comment.skip)
                    continue;
                const classMatch = lines[i].match(/(?:abstract\s+)?(?:interface\s+)?(?:final\s+|sealed\s+|base\s+)?class\s+([A-Za-z_]\w*)/);
                if (!classMatch)
                    continue;
...
function isInsideComment(line, inBlock) {
    ...
    if (inBlock) {
        if (trimmed.includes("*/"))
            return { skip: true, inBlock: false };
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (identifyWordNumber(lower) === "plural") {
                ...
                (0, _types_1.sendFormattedFail)({
                    ...
                    objective: "Nomes de classes devem ser **singulares** — o plural é reservado para variáveis do tipo `List`.",
                    file,
                    line: i + 1,
                });
            }
