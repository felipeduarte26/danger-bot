            ...
            if (fileText.match(/class\s+\w+\s+extends\s+(StatefulWidget|StatelessWidget|State)</)) {
                ...
                lines.forEach((line, idx) => {
                    if (line.trim() === '@override')
                        lastOverrideLine = idx;
                    if (line.match(/^\s*(?:void|Future|Widget)\s+\w+\s*\(/) && !line.includes('@override')) {
                        if (firstNonOverrideLine === -1)
                            firstNonOverrideLine = idx;
                    }
                });
                if (firstNonOverrideLine > 0 && lastOverrideLine > firstNonOverrideLine) {
                    (0, _types_1.sendFail)(`## 📱 ORDEM DE FUNÇÕES INCORRETA
