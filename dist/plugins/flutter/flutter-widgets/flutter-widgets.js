        ...
        for (const cls of classes) {
            ...
            for (const violator of violators) {
                (0, _types_1.sendFormattedFail)({
                    ...
                    objective: "O conceito de **Vertical Ordering** (Clean Code, Cap. 5 — Robert C. Martin) diz que o código deve parecer um jornal: o leitor entende o contexto geral antes de se aprofundar nos detalhes técnicos.",
                    reference: {
...
const WIDGET_METHOD_WITH_TYPE_RE = /^\s+(?:static\s+)?(?:Future<[^>]*(?:<[^>]*>)*>|Stream<[^>]*(?:<[^>]*>)*>|void|bool|int|double|String|Widget|List<[^>]*>|Map<[^>]*>|Set<[^>]*>|[A-Za-z_][\w<>,? ]*)\s+([a-zA-Z_]\w*)\s*[(<]/;
const WIDGET_METHOD_NAME_ONLY_RE = /^\s+([a-zA-Z_]\w*)\s*[(<]/;
function hasOverrideAbove(lines, idx) {
    for (let k = idx - 1; k >= Math.max(0, idx - 5); k--) {
        ...
        if (t === "" ||
            ...
            t.startsWith("@"))
            continue;
        if (!t.includes("=") && !t.endsWith(";") && !t.endsWith("{") && !t.endsWith("}"))
            continue;
        break;
    }
    return false;
}
function extractMethods(lines, startLine, endLine, className) {
    const methods = [];
    let depth = 0;
    ...
    for (let i = startLine; i <= endLine; i++) {
        ...
        if (eqSign !== -1 && (firstParen === -1 || eqSign < firstParen))
            continue;
        ...
        if (trimmed.endsWith(";") && /\)\??\s+\w+\s*;$/.test(trimmed))
            continue;
