...
function hasAssignment(lines, idx) {
    ...
    if (/^(final|var|const)\s/.test(currentTrimmed))
        return true;
    if (/\w+\s*=\s*await/.test(currentTrimmed))
        return true;
    if (/^]\s*=\s*await/.test(currentTrimmed)) {
        for (let j = idx - 1; j >= Math.max(0, idx - 8); j--) {
            ...
            if (prev === "")
                continue;
            if (/^(final|var|const)\s+\[/.test(prev))
                return true;
...
exports.default = (0, _types_1.createPlugin)({
    ...
    for (const file of dartFiles) {
        ...
        for (let i = 0; i < lines.length; i++) {
            if (skip[i])
                continue;
            if (!FUTURE_WAIT_RE.test(lines[i]))
                continue;
            if (!isLiteralListArg(lines, i))
                continue;
            if (!hasAssignment(lines, i))
                continue;
            ...
            (0, _types_1.sendFormattedFail)({
                ...
                description: "`Future.wait([...])` detectado. Prefira a sintaxe de **tupla com `.wait`** do Dart 3 — type-safe, sem necessidade de cast por índice e mais legível.",
                problem: {
                    wrong: wrongSnippet,
                    correct: correction ||
                        `final (\n  resultA,\n  resultB,\n) = await (\n  futureA(),\n  futureB(),\n).wait;`,
                ...
                objective: "Usar a sintaxe moderna de tupla com `.wait` para **type-safety**, eliminando casts manuais e acesso por índice.",
                reference: {
