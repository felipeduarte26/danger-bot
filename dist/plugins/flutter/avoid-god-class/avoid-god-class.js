...
function countConstructorLines(lines, className, startIdx, endIdx) {
    ...
    for (let i = startIdx; i <= endIdx; i++) {
        ...
        if (depthBeforeLine !== 1 || !pattern.test(trimmed))
            continue;
        total++;
        ctorParenDepth = 0;
    ...
    function isCtorDone(trimmedLine) {
        if (!ctorParamsComplete)
            return false;
        if (!ctorFoundBody && trimmedLine.endsWith(";"))
            return true;
        if (ctorFoundBody && ctorBodyBraceDepth <= 0)
            return true;
        return false;
    }
}
...
const METHOD_WITH_TYPE_RE = /^\s+(?:static\s+)?(?:Future<[^>]*(?:<[^>]*>)*>|Stream<[^>]*(?:<[^>]*>)*>|void|bool|int|double|String|List<[^>]*>|Map<[^>]*>|Set<[^>]*>|[A-Za-z_][\w<>,? ]*)\s+([a-z]\w*)\s*[(<]/;
...
function parseClasses(lines) {
    ...
    while (i < lines.length) {
        ...
        const classMatch = line.match(/^\s*(?:abstract\s+)?(?:final\s+)?(?:base\s+)?(?:sealed\s+)?(?:interface\s+)?class\s+([A-Za-z_]\w*)/);
        if (classMatch) {
    ...
    for (const file of dartFiles) {
        ...
        for (const cls of classes) {
            ...
            if (effectiveLineCount > maxLines) {
                const lineDetail = isViewModel && cls.constructorLineCount > 0
                ...
                (0, _types_1.sendFormattedFail)({
                    ...
                    objective: "Seguir o **Princípio de Responsabilidade Única** (SRP) — cada classe deve ter um único motivo para mudar.",
                    reference: {
