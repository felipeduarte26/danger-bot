...
function sortBySeverity(points) {
    const severity = (p) => {
        if (p.includes("\u{1F534}"))
            return 0; // 🔴
        if (p.includes("\u{1F7E1}"))
            return 1; // 🟡
        if (p.includes("\u{1F535}"))
            return 2; // 🔵
        return 3;
    };
...
function splitPoints(text) {
    ...
    for (const line of lines) {
        const isNewPoint = /^\s*(?:[•\-*]|\u{1F534}|\u{1F7E1}|\u{1F535})\s/u.test(line) || /^\s*\d+[.)]\s/.test(line);
...
exports.default = (0, _types_1.createPlugin)({
    ...
    if (apiKeys.length === 0) {
        console.log("⚠️ ai-code-review: nenhuma API key configurada. " +
            "Defina gemini_api_keys no danger-bot.yaml ou GEMINI_API_KEYS/GEMINI_API_KEY como variável de ambiente.");
        return;
    }
    const { git } = (0, _types_1.getDanger)();
    const dartFiles = [...git.modified_files, ...git.created_files].filter((f) => f.endsWith(".dart") && shouldAnalyzeFile(f) && fs.existsSync(f));
    ...
    console.log(`🤖 AI Code Review: ${dartFiles.length} arquivo(s) .dart encontrados` +
    ...
    for (let i = 0; i < filesToAnalyze.length; i++) {
        ...
        if (usedWithCurrentKey >= FILES_PER_KEY && keyIndex < apiKeys.length - 1) {
            ...
            console.log(`  🔑 Trocando para key ${keyIndex + 1}/${apiKeys.length} ...${apiKeys[keyIndex].slice(-6)}`);
        }
        ...
        if (result.rateLimited) {
            ...
            console.log(`  ⚠️ Rate limit na key ...${apiKeys[keyIndex].slice(-6)} (${consecutiveRateLimits}/${MAX_CONSECUTIVE_RATE_LIMITS})`);
            await sleep(2000);
            if (consecutiveRateLimits >= MAX_CONSECUTIVE_RATE_LIMITS) {
                ...
                console.log("  🛑 Todas as keys atingiram rate limit consecutivo. Interrompendo AI Code Review para não travar a pipeline.");
                aborted = true;
                break;
            }
    ...
    if (reviewed > 0) {
        ...
        console.log(`⚠️ ai-code-review: nenhum arquivo analisado com sucesso (${skipped} falharam/ignorados).`);
    }
});
