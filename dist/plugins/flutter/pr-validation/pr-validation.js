...
exports.default = (0, _types_1.createPlugin)({
    ...
    if (pubspecLockChanged && !pubspecYamlChanged) {
        (0, _types_1.sendFormattedFail)({
            ...
            description: "O `pubspec.lock` foi alterado mas o `pubspec.yaml` não. Pode indicar inconsistência de dependências.",
            problem: {
    ...
    if (linesChanged === 0 && totalFiles === 0) {
        ...
        (0, _types_1.sendMessage)(`✅ **Ótimo**: PR pequeno e focado (**${linesChanged} linhas** em ${totalFiles} arquivo(s))`);
    }
    else if (linesChanged <= 200) {
        (0, _types_1.sendMessage)(`👍 **Bom**: PR de tamanho médio (**${linesChanged} linhas** em ${totalFiles} arquivo(s))`);
    }
    else if (linesChanged <= 600) {
        (0, _types_1.sendWarn)(`⚠️ **Atenção**: PR grande (**${linesChanged} linhas** em ${totalFiles} arquivo(s)). Considere quebrar em PRs menores.`);
    }
    else {
        (0, _types_1.sendWarn)(`🚨 **PR Muito Grande**: **${linesChanged} linhas** em ${totalFiles} arquivo(s)! Forte recomendação de quebrar em múltiplos PRs menores.`);
    }
});
