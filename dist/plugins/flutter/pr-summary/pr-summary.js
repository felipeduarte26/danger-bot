...
function categorizeFiles(files) {
    ...
    for (const f of files) {
        ...
        else if (f.match(/pubspec\.(yaml|lock)$|\.gradle|Podfile|analysis_options|\.env|\.properties/)) {
            cats[3].files.push(f);
        }
...
function getSizeInfo(dartCount) {
    if (dartCount === 0)
        return { label: "Sem Dart", emoji: "⚪" };
    if (dartCount <= 5)
        return { label: "Pequena", emoji: "🟢" };
    if (dartCount <= 15)
        return { label: "Média", emoji: "🟡" };
    if (dartCount <= 40)
        return { label: "Grande", emoji: "🟠" };
...
function assessRisk(dartSource, testFiles, layers, categories) {
    ...
    if (count >= 3)
        return { level: "Alto", emoji: "🔴", factors };
    if (count >= 2)
        return { level: "Médio", emoji: "🟡", factors };
