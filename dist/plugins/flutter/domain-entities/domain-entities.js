 ...
 */
function headerFieldsByLine(content) {
  const result = new Map();
  for (const [lineIndex, fields] of (0, primary_constructors_1.primaryConstructorFieldsByLine)(
    content
  )) {
    result.set(
      lineIndex,
...
function isEntityExtendedByModel(entityFilePath, entityClassName, entityFields) {
  ...
  const modelLines = (0, primary_constructors_1.normalizePrimaryConstructorHeaders)(
    modelContent
  ).split("\n");
  ...
  async () => {
    ...
    for (const file of files) {
      ...
      if (subfolder && VALID_ENUM_PARENTS.has(subfolder)) {
        ...
        validateEnum(file, (0, primary_constructors_1.normalizePrimaryConstructorHeaders)(content));
        continue;
      }
      const content = fs.readFileSync(file, "utf-8");
      const normalized = (0, primary_constructors_1.normalizePrimaryConstructorHeaders)(content);
