...
function buildSkipMask(lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (inMultiLineString) {
            skip[i] = true;
            if (lines[i].includes(stringDelimiter))
...
function classifyTry(lines, start) {
    ...
    for (let i = start; i < lines.length; i++) {
        ...
        if (i > start && opened) {
            ...
            if (CATCH_RE.test(trimmed))
                hasCatch = true;
            if (FINALLY_RE.test(trimmed))
                hasFinally = true;
        }
        if (opened && braces <= 0)
            break;
    ...
    for (const file of dartFiles) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            (0, _types_1.sendFormattedFail)({
                ...
                objective: "Manter **separação de responsabilidades**: erros são tratados onde ocorrem (Data/Domain), não onde são exibidos (Presentation).",
                reference: {
