...
function findStateClasses(lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        const classMatch = line.match(/^\s*(?:final\s+)?(?:abstract\s+)?(?:base\s+)?(?:sealed\s+)?class\s+(\w+)/);
        if (!classMatch)
            continue;
...
function hasSkipAnnotation(lines, idx) {
    for (let k = idx - 1; k >= Math.max(0, idx - 10); k--) {
        ...
        if (SKIP_ANNOTATIONS.some((a) => t.startsWith(a)))
            return true;
...
function isBuilderPattern(lines, stateClass) {
    ...
    for (let i = stateClass.startLine; i <= stateClass.endLine; i++) {
        ...
        if (trimmed.startsWith("//") || trimmed.startsWith("///"))
            continue;
        if (WIDGET_CALL_SINGLE_LINE.test(lines[i]))
            return true;
...
function findPublicMembers(lines, stateClass) {
    ...
    for (let i = stateClass.startLine; i <= stateClass.endLine; i++) {
        ...
        if (!classBodyStarted || depthBefore !== 1)
            continue;
        if (trimmed.length === 0)
            continue;
        ...
        if (hasSkipAnnotation(lines, i))
            continue;
        if (/\bstatic\b/.test(trimmed))
            continue;
        if (new RegExp(`^(?:const\\s+)?(?:factory\\s+)?${stateClass.className}(?:\\.[a-zA-Z_]\\w*)?\\s*\\(`).test(trimmed))
            continue;
...
exports.default = (0, _types_1.createPlugin)({
    ...
    description: "Detecta membros públicos em State classes da presentation que deveriam ser privados",
    enabled: true,
}, async () => {
    const { git } = (0, _types_1.getDanger)();
    const allFiles = [...git.created_files, ...git.modified_files];
    const dartPresentationFiles = allFiles.filter((f) => f.endsWith(".dart") &&
    ...
    console.log(`[presentation-encapsulation] Arquivos: total=${allFiles.length} presentation=${dartPresentationFiles.length} existentes=${dartFiles.length}`);
        ...
        try {
            ...
            for (const stateClass of stateClasses) {
                ...
                if (isBuilderPattern(lines, stateClass)) {
                    skippedBuilder++;
                    console.log(`[presentation-encapsulation] Builder pattern detectado em ${stateClass.className} (${file})`);
                    continue;
                }
                const publicMembers = findPublicMembers(lines, stateClass);
                if (publicMembers.length === 0)
                    continue;
                console.log(`[presentation-encapsulation] ${stateClass.className}: ${publicMembers.length} membro(s) público(s) em ${file}`);
                for (const member of publicMembers) {
                    if (hasIgnoreComment(lines, member.line - 1))
                        continue;
                    totalViolations++;
                    ...
                    (0, _types_1.sendFormattedFail)({
                        ...
                        objective: "Membros de **State** devem ser **privados** (`_`). O State é um detalhe de implementação — sua API não deve vazar para fora do widget.",
                        file,
                        line: member.line,
                    });
                }
    ...
    console.log(`[presentation-encapsulation] Resultado: stateClasses=${totalStateClasses} builder=${skippedBuilder} ignore=${skippedIgnore} violações=${totalViolations} erros=${fileErrors}`);
    if (totalViolations > 0) {
        (0, _types_1.sendMessage)(`**Presentation Encapsulation**: ${totalViolations} membro(s) público(s) em State classes que deveria(m) ser privado(s)`);
    }
});
