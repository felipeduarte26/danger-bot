...
async function loadSpell() {
    ...
    try {
        ...
        const dictMod = await Promise.resolve(`${"dictionary-pt"}`).then(s => __importStar(require(s)));
    ...
    while (i < len) {
        ...
        if (line[i] === "r" &&
            ...
            if (i + 1 < len && line[i] === q && line[i + 1] === q) {
                i += 2;
                while (i + 2 < len && !(line[i] === q && line[i + 1] === q && line[i + 2] === q))
                    i++;
                i += 3;
            }
            else {
...
function isNonTextContext(line, str) {
    ...
    const q = `['"]${escaped}['"]`;
    if (new RegExp(`${q}\\s*:`).test(line))
        return true;
    if (new RegExp(`\\[\\s*${q}\\s*]`).test(line))
        return true;
    if (/\b(?:Key|ValueKey|GlobalKey|ObjectKey|PageStorageKey)\s*\(/.test(line))
        return true;
    return false;
}
...
function isSkippableWord(word) {
    ...
    if (SKIP_WORDS.has(word.toLowerCase()))
        return true;
    if (/^[A-Z][a-z]{0,2}$/.test(word))
        return true;
    if (/^[a-z]+ed$/.test(word))
        return true;
    if (/^[a-z]+ing$/.test(word))
        return true;
    if (/^[a-z]+tion$/.test(word))
        return true;
    if (/^[a-z]+ment$/.test(word))
        return true;
    if (/^[a-z]+ness$/.test(word))
        return true;
    if (/^[a-z]+able$/.test(word))
        return true;
    if (/^[a-z]+ible$/.test(word))
        return true;
    if (/^[a-z]+ful$/.test(word))
        return true;
    if (/^[a-z]+less$/.test(word))
        return true;
    if (/^[a-z]+ous$/.test(word))
        return true;
    if (/^[a-z]+ive$/.test(word))
        return true;
    if (/^[a-z]+ly$/.test(word))
        return true;
    return false;
...
function isSkippableLine(line) {
    ...
    if (/^\s*@\w+/.test(trimmed))
        return true;
    if (/\bassert\s*\(/.test(trimmed))
        return true;
    if (/\b(?:debugPrint|print|log\.(?:d|i|w|e|v))\s*\(/.test(trimmed))
        return true;
    if (/\bRegExp\s*\(/.test(trimmed))
        return true;
    if (/\bcase\s+'[^']*'\s*:/.test(trimmed))
        return true;
    if (/\bcase\s+"[^"]*"\s*:/.test(trimmed))
        return true;
    return false;
...
async function checkWordWithNodehun(word, spell) {
    ...
    const closeSuggestions = suggestions.filter((s) => levenshtein(lower, s.toLowerCase()) <= maxDist);
...
function checkBlindSpots(word) {
    ...
    if (blind)
        return { suggestions: [blind], isAccentError: true };
    return null;
}
...
exports.default = (0, _types_1.createPlugin)({
    ...
    const dartFiles = [...danger.git.created_files, ...danger.git.modified_files].filter((f) => f.endsWith(".dart") &&
    ...
    for (const file of dartFiles) {
        ...
        for (const chunk of diff.chunks) {
            for (const change of chunk.changes) {
                ...
                if (isSkippableLine(lineContent))
                    continue;
                const strings = extractStringLiterals(lineContent);
                for (const str of strings) {
                    if (isNonTextString(str))
                        continue;
                    if (isNonTextContext(lineContent, str))
                        continue;
                    const words = str
                        .split(/[\s.,;:!?()[\]{}<>/\\|@#$%^&*+=~`"'0-9]+/)
                        .filter((w) => w.length >= 2);
                    for (const word of words) {
                        ...
                        const isDup = existing.some((e) => e.word === word.toLowerCase() && e.line === lineNum);
                        if (!isDup) {
                            existing.push({
        ...
        for (const err of limited) {
            if (err.isAccentError) {
                ...
                (0, _types_1.sendFormattedFail)({
                    ...
                    objective: "Textos visíveis ao usuário devem estar com **ortografia correta** em português.",
                    file,
                    line: err.line,
                });
            }
    ...
    if (totalErrors > 0) {
        (0, _types_1.sendMessage)(`**Spell Check PT-BR**: ${totalErrors} problema(s) de ortografia em ${errorsByFile.size} arquivo(s)`);
    }
});
