...
function commentContainsPtWords(text) {
    ...
    const ptFound = words.filter((w) => {
        ...
        return ((PT_WORDS.has(w) || PT_WORDS.has(normalized)) &&
            !AMBIGUOUS.has(w) &&
...
exports.default = (0, _types_1.createPlugin)({
    ...
    for (const c of commentMatches) {
        ...
        (0, _types_1.sendFormattedFail)({
            ...
            objective: "Manter **consistência** com o ecossistema (SDK, docs, frameworks) e facilitar **colaboração** entre equipes.",
            reference: {
    ...
    for (const m of matches) {
        ...
        if (seen.has(key))
            continue;
        seen.add(key);
        const currentDecl = m.kind === "classe"
            ? `class ${m.identifier}`
            : m.kind === "método"
                ? `void ${m.identifier}()`
                : `final ${m.identifier}`;
        const suggestedDecl = m.kind === "classe"
...
function extractIdentifiers(line) {
    ...
    const methodRe = /(?:Future<[^>]*>|void|String|int|double|bool|num|dynamic|List<[^>]*>|Map<[^,>]*,[^>]*>|Set<[^>]*>|[A-Z]\w*(?:<[^>]*>)?)\s+([a-z_]\w*)\s*\(/g;
    while ((m = methodRe.exec(line)) !== null) {
        if (!isReserved(m[1]))
            results.push({ identifier: m[1], kind: "método" });
    }
    const varRe = /(?:final|const|var|late\s+final|late)\s+(?:[A-Za-z_]\w*(?:<[^>]*>)?\s+)?([a-z_]\w*)\s*[=;]/g;
    while ((m = varRe.exec(line)) !== null) {
        if (!isReserved(m[1]))
            results.push({ identifier: m[1], kind: "variável" });
    }
    return results;
}
