            ...
            for (const stmt of statements) {
                for (const pattern of HEAVY_PATTERNS) {
                    if (pattern.regex.test(stmt.text)) {
                        const displayCode = stmt.text.length > 120 ? stmt.text.substring(0, 117) + "..." : stmt.text;
...
function findAllBuildMethods(lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
        if (!lines[i].match(/Widget\s+build\b\s*\(/))
            continue;
...
function extractPreReturnStatements(lines, buildStart) {
    if (isArrowFunction(lines, buildStart))
        return [];
    const statements = [];
    let depth = 0;
    let foundBody = false;
    ...
    for (let i = buildStart; i < lines.length; i++) {
        ...
        if (currentStmtParts.length === 0) {
            if (!trimmed || trimmed.startsWith("//") || trimmed.startsWith("///"))
                continue;
            if (depthBefore === 1 && isReturnLine(trimmed))
                break;
        }
        if (trimmed.length > 0) {
            if (stmtStartLine === -1)
