...
exports.default = (0, _types_1.createPlugin)({
    ...
    for (const file of dartFiles) {
        ...
        if (!content.includes("Widget build("))
            continue;
        ...
        for (const body of buildBodies) {
            for (let i = body.start; i <= body.end; i++) {
                ...
                if (isInsideMultiLineString(lines, i))
                    continue;
                ...
                (0, _types_1.sendFormattedFail)({
                    ...
                    objective: "Comentários `///` dentro de `build` poluem o código e não geram documentação útil. Widgets são auto-descritivos por seus nomes e parâmetros.",
                    file,
                    line: i + 1,
                });
            }
 ...
 */
function findBuildBodies(lines) {
    const bodies = [];
    for (let i = 0; i < lines.length; i++) {
        const trimmed = lines[i].trim();
        if (!/Widget\s+build\s*\(/.test(trimmed))
            continue;
        if (!hasOverrideAbove(lines, i))
            continue;
 ...
 */
function buildRealSnippets(lines, docLineIdx) {
 ...
 */
function isInsideMultiLineString(lines, lineIdx) {
