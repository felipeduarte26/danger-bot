...
exports.default = (0, _types_1.createPlugin)({
    ...
    const files = [...git.created_files, ...git.modified_files].filter((f) => f.includes("/failures/") &&
        f.endsWith(".dart") &&
    ...
    for (const file of files) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const finalMatch = lines[i].match(/final\s+class\s+([A-Za-z_]\w*)\s+extends\s+([A-Za-z_]\w*)/);
            if (finalMatch) {
        ...
        if (!sealedClass) {
            (0, _types_1.sendFormattedFail)({
                ...
                description: "Arquivo de Failure deve ter uma `sealed class` como classe base para pattern matching exaustivo.",
                problem: {
